...
const service_data = require('./public/service_data.js'); //using data from service_data.js
...
app.post("/login.html", function (req, res) {
  ...
  newUsername = req.body.username.toLowerCase(); //username entered is case insensitive, assign to variable the_username
  if (typeof users_reg_data[newUsername] != 'undefined') { //check if the username exists in the json data
    if (users_reg_data[newUsername].password == req.body.password) { //make sure password matches exactly - case sensitive
    req.query.username = newUsername; //adding the case insensitive username to the query
      ...
      res.redirect('/checkout.html?' + querystring.stringify(req.query)); //keeping the querystring when redirecting to the invoice
      ...
      errors.push = ('Invalid Password'); //push login error for invalid password
      console.log(LogErrors); //console log error to check working
      req.query.username= newUsername; //add username to querystring
      req.query.password=req.body.password; //add password to querystring
      req.query.LogErrors = LogErrors.join(';'); //joining the login errors for the querystring
    }
  }
  else { //if username does not exist in registration data
    LogErrors.push = ('Invalid Username'); //push login error for invalid username
    console.log(LogErrors); //console log error to check working
    req.query.username= newUsername; //add username to querystring
    req.query.password=req.body.password; //add password to querystring
    req.query.LogErrors=LogErrors.join(';'); //joining login errors for querystring
  }
...
app.post("/registration.html", function (req, res) {
  ...
  var pswderrors = []; //to store password errors
  ...
  if ((req.body.password.length < 6)) { //if password length is less than 6 characters
    pswderrors.push('Password Too Short') //push to password error array
  ...
  if (pswderrors.length == 0) { //if have password errors
    console.log('no password errors!'); //to make sure if statement working
  }
  if (pswderrors.length > 0) { //if have password errors
    console.log('error:'+ pswderrors) //console log password errors
    req.query.pswderrors = pswderrors.join(';'); //joining password errors together
  }
  ...
  if (errors.length == 0) { //if there are no errors
    ...
    res.redirect('./checkout.html?' + querystring.stringify(req.query)) //redirect to the invoice
  }
  //add errors to querystring (for purpose of putting back into textbox)
    ...
    res.redirect('./registration.html?' + querystring.stringify(req.query)) //trying to add query from registration page and invoice back to register page on reload
  }
}
);
...
app.get('/purchase', function (req, res, next) { //getting the data from the form where action is '/purchase' 
  ...
  for (i = 0; i < service_data.length; i++) { // for every service in the array, increasing by 1
    a_qty = GET['qtyTextbox' + i]; // q is equal to the quantity pulled from what is entered into the textbox
    if (isNonNegInt(q) == false) { //if the quantity is not an integer
      hasValidQuantities = false; //hasValidQuantities is false 
    }
    if (a_qty > 0) { // if the quantity entered in textbox is greater than 0
...
//lab11 check to see if a_qty is a non-negative integer
       ...
       function isNonNegInt(a_qty, sendArrayBack = false) {  //Function from lab 11
        errors = []; // assume no errors at first
            if (Number(a_qty) != q) errors.push('Not a number!'); // Check if string is a number value
            if (a_qty < 0) errors.push('Negative value!'); // Check if it is non-negative
            if (parseInt(a_qty) != a_qty) errors.push('Not an integer!'); // Check that it is an integer
